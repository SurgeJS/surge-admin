...
// 默认占位符
const defaultPlaceholder = (label: Schema['label']) => {
...
// 生成placeholder
export const generatePlaceholder = (label: Schema['label'], component: ComponentsName, type?: string) => {
...
export const generateRule = (label: Schema['label'], component: ComponentsName): FormItemRule => {
    ...
    if (isInputComponent) message = placeholder['input']
    else if (isSelectComponent) message = placeholder['pick']
...
// 处理规则预设
export const handleRulePresets = (rule: RulePresets): FormItemRule => ({
    required: true,
