import {ColProps, FormProps, RowProps} from 'ant-design-vue'
...
/**
 * 布局
 * @description query 查询表单
 * @description module 模块表单
 * @description step 步骤表单
 */
export type SchemaLayout = 'query' | 'module' | 'step'
...
// 模块表单结构
export interface ModuleSchemaType<T extends Recordable = any, C extends ComponentsName = ComponentsName> {
    // 模块标题
    title: string
...
// 步骤条表单结构
export interface StepSchemaType<T extends Recordable = any, C extends ComponentsName = ComponentsName> {
    // 步骤标题
    title: string
